...
const AnimatedBackground = () => {
  ...
  // Reverse mapping: skill name → Spline keycap object name
  // Used when we need to find the 3D keycap for a skill whose name
  // differs from the Spline object (e.g. "angular" → keycap is "react")
  const skillToKeycapName: Record<string, string> = {
    angular: "react",
    "Spring Boot": "vue",
  };

  const findKeycapForSkill = (skill: { name: string }) => {
    const keycapName = skillToKeycapName[skill.name] || skill.name;
    return splineApp?.findObjectByName(keycapName) ?? null;
  };
  ...
  const handleMouseHover = (e: SplineEvent) => {
    ...
    const hoveredSkill = resolveSkillFromTarget(e.target as unknown as SPEObject);
  ...
  const handleSplineInteractions = () => {
    ...
    const handleKeyDown = (e: SplineEvent) => {
      if (!splineApp || isInputFocused()) return;
      const skill = resolveSkillFromTarget(e.target as unknown as SPEObject);
      if (skill) {
        playPressSound();
  ...
  const getKeycapsAnimation = () => {
    ...
    const start = () => {
      ...
      Object.values(SKILLS)
        ...
        .forEach((skill, idx) => {
          const keycap = findKeycapForSkill(skill);
          if (!keycap) return;
